...
var func = function (timer, tabid1, test) {
    ...
    var refreshPBIRS = document.getElementsByClassName('pbi-glyphicon glyphui pbi-glyph-refresh');
    if (!refreshPBIRS === false)
        $(refreshPBIRS).click();
...
chrome.runtime.onMessage.addListener(
    function (request, sender, sendResponse) {
        ...
        if (request.msg == "iamready") {
            ...
            if ((!refresh || refresh.length < 1) && (!refreshPBIRS || refreshPBIRS.length < 1) && (!refreshDashboard)) sendResponse(false); else sendResponse(true);
